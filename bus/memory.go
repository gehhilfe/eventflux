...
type InMemoryMessageBus struct {
	subscriptions map[string][]func(message []byte, metadata map[string]string) error
}

func NewInMemoryMessageBus() *InMemoryMessageBus {
	return &InMemoryMessageBus{
		subscriptions: make(map[string][]func(message []byte, metadata map[string]string) error),
	}
}

func (b *InMemoryMessageBus) Publish(subject string, message []byte) error {
	for _, handler := range b.subscriptions[subject] {
		if err := handler(message, map[string]string{}); err != nil {
			return err
		}
	}
	return nil
...
func (b *InMemoryMessageBus) Subscribe(subject string, handler func(message []byte, metadata map[string]string) error) (core.Unsubscriber, error) {
...
type unsubscriber struct {
	...
	subject string
	handler *func(message []byte, metadata map[string]string) error
}
